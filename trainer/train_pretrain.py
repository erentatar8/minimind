...
if __name__ == "__main__":
    ...
    default_data = "../dataset/pretrain_turkce.jsonl" if os.path.exists("../dataset/pretrain_turkce.jsonl") else "dataset/pretrain_turkce.jsonl"
    default_ckp = "../checkpoints" if os.path.exists("../checkpoints") else "checkpoints"

    parser = argparse.ArgumentParser(description="MiniMind Pretraining (Ön Eğitim)")
    parser.add_argument("--save_dir", type=str, default=default_out, help="Model kayıt dizini")
    parser.add_argument('--save_weight', default='pretrain', type=str, help="Kayıt ağırlık dosya öneki")
    parser.add_argument("--epochs", type=int, default=1, help="Eğitim tur (epoch) sayısı")
    parser.add_argument("--batch_size", type=int, default=16, help="Batch boyutu")
    parser.add_argument("--learning_rate", type=float, default=5e-4, help="Başlangıç öğrenme oranı (learning rate)")
    ...
    parser.add_argument("--accumulation_steps", type=int, default=4, help="Gradyan biriktirme adımı")
    parser.add_argument("--grad_clip", type=float, default=1.0, help="Gradyan kırpma (clipping) eşiği")
    parser.add_argument("--log_interval", type=int, default=50, help="Log yazdırma sıklığı (adım)")
    ...
    parser.add_argument('--max_seq_len', default=340, type=int, help="Maksimum dizi uzunluğu (sequence length)")
    ...
    parser.add_argument("--data_path", type=str, default=default_data, help="Ön eğitim veri seti yolu")
    parser.add_argument('--from_weight', default='none', type=str, help="Hangi ağırlıktan başlanacağı (sıfırdan başlamak için none)")
    ...
    parser.add_argument("--wandb_project", type=str, default="MiniMind-Pretrain", help="Wandb proje adı")
